...
# Custom CSS to match your branding and HTML tool's results box
st.markdown(f"""
    <style>
...
# 3. Input Organization (Combining 88 Questions with Logic)
...
with tab1:
    ...
    with col_id:
        ...
        dob = st.text_input("Date of Birth (mm/dd/yyyy)") [cite: 70]
        sex = st.radio("Sex", ["Male", "Female", "Other"]) [cite: 65]
        citizen = st.radio("U.S. Citizen?", ["Yes", "No"]) [cite: 311]
    ...
    with col_phys:
        ...
        weight = st.number_input("Weight (lbs)", value=180) [cite: 299]
        
        st.markdown("---")
        nic = st.selectbox("Nicotine Use", ["Never / >3 years clean", "1-3 years clean", "Current / Within 12 months"]) [cite: 303]
        weed = st.selectbox("Marijuana Use", ["None", "Occasional / Recreational", "Medical Prescription", "Daily Use (>5x week)"]) [cite: 217]

with tab2:
    st.subheader("Comprehensive Risk Profile")
    st.write("Select all health history items that apply to ensure a precise pre-underwriting analysis.") [cite: 54]
    ...
    with col_mild:
        ...
        m3 = st.checkbox("Asthma / Sleep Apnea / Migraines") [cite: 79]
        m4 = st.checkbox("Anxiety / Depression / PTSD") [cite: 103]
        m5 = st.checkbox("Diabetes Type 2 (Oral Meds)") [cite: 125]
        
    with col_sev:
        st.markdown("##### 🟠 Severe / Substandard")
        s1 = st.checkbox("Diabetes (Insulin / Complications)") [cite: 125]
        s2 = st.checkbox("Heart Attack / Stent / Angina") [cite: 260]
        s3 = st.checkbox("Stroke / TIA / A-Fib") [cite: 144]
        s4 = st.checkbox("Past History of Cancer") [cite: 89]
        s5 = st.checkbox("Bipolar / Suicide Attempt") [cite: 103]
        
    with col_ko:
        st.markdown("##### 🔴 Knockouts / GI Only")
        k1 = st.checkbox("Organ Transplant / ICD") [cite: 265]
        k2 = st.checkbox("ALS / Alzheimer's / Dementia") [cite: 144]
        k3 = st.checkbox("COPD / Emphysema / Oxygen") [cite: 79]
        k4 = st.checkbox("Kidney Failure / Dialysis") [cite: 92]
        k5 = st.checkbox("Active/Current Cancer Treatment") [cite: 89]

with tab3:
    st.subheader("Lifestyle & Background")
    col_l1, col_l2 = st.columns(2)
    with col_l1:
        income = st.number_input("Annual Income", value=75000) [cite: 213]
        occ = st.text_input("Occupation") [cite: 82]
        military = st.radio("Armed Forces / Reserves?", ["Yes", "No"]) [cite: 250]
    with col_l2:
        risk_hobbies = st.checkbox("High-Risk Hobbies (Skydiving, Racing, etc.)") [cite: 349]
        pilot_license = st.checkbox("Pilot License / Flown in past 5 years") [cite: 353]
        criminal = st.checkbox("Criminal Offense / Parole history") [cite: 256]
        bankruptcy = st.checkbox("Bankruptcy / Bad Debt (Last 5 years)") [cite: 359]

with tab4:
    st.subheader("Physician Data")
    st.text_input("Primary Physician Name") [cite: 367]
    st.text_area("Reason last seen and results of visit") [cite: 370]
    st.text_area("List all current Medications & Dosages") [cite: 284]

# --- 4. UNDERWRITING LOGIC (FROM YOUR HTML TOOL) ---
# Calculate BMI
...
# Base Class Logic
hClass = "Standard"
cRank = 4 # 1:Pref+, 2:Pref, 3:Std+, 4:Std, 5:Table, 6:Decline
badge_color = "#f1c40f" # Default Yellow
...
with col_res2:
    ...
    with st.container():
        ...
        if cRank <= 4:
            ...
            st.write("* Day-One Coverage Eligible")
        elif cRank == 5:
            ...
            st.write("* Ineligible for Term or IUL due to severe risk history.")
        st.markdown('</div>', unsafe_allow_html=True)

st.divider()
st.link_button("Submit Data for Fiduciary Review", "https://lifepolicypilot.blog/contact-2/")
st.caption("Disclaimer: This tool provides general estimates based on universal field underwriting guidelines. Final risk classification is subject to formal carrier review and background checks.")
